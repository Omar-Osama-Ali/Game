...
const FVUi = (function () {
  ...
  function renderSetup() {
    ...
    if (draft.region === "custom") {
      const customSet = new Set((draft.customCountries || []).map((c) => c.toUpperCase()));
      el("setup-custom-grid").innerHTML = COUNTRIES.slice()
        .sort((a, b) => a.name.localeCompare(b.name))
        .map(
          (c) =>
            `<button class="fv-choice${customSet.has(c.code) ? " fv-selected" : ""}" data-code="${c.code}">${c.name}</button>`
        )
        .join("");
  ...
  function handleMapAnswer(q, country, feature) {
    ...
    d3.select("#quiz-map-svg").selectAll("path.fv-country").classed("fv-interactive", false).on("click", null);
